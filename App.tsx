...
function AppContent() {
            ...
            try {
                ...
                if (file.file.name.toLowerCase().endsWith('.json')) {
                    ...
                    if (fileBase64Encoded.length > 4 * 1024 * 1024) {
                        ...
                        const blob = await blobUpload(file.file.name, blobFile, {
                            access: 'public',
    ...
    const executeRagIngest = async (fileIds: string[], folderName?: string) => {
        ...
        for (const file of filesToIngest) {
            try {
                ...
                // Si el base64 supera ~4MB, subir directo a Vercel Blob desde el navegador
    ...
    return (
        <Routes>
            ...
            <Route path="/unprocessable" element={<UnprocessablePage isDarkMode={isDarkMode} />} />
            {/* ✅ Nuevas rutas */}
            <Route path="/rag" element={<BibliotecaPage isDarkMode={isDarkMode} />} />
            <Route path="/guia" element={<UserGuidePage isDarkMode={isDarkMode} />} />
            {/* Admin Dashboard */}
            <Route
                path="/admin"
