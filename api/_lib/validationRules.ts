...
/**
 * Códigos IATA de ciudades españolas
 */
const CODIGOS_IATA = {
  'MAD': 'Madrid', 'BCN': 'Barcelona', 'VLC': 'Valencia', 'SVQ': 'Sevilla',
  'ZGZ': 'Zaragoza', 'MLG': 'Málaga', 'BIO': 'Bilbao', 'PMI': 'Palma',
  'ALC': 'Alicante', 'COR': 'Córdoba', 'VLL': 'Valladolid', 'VGO': 'Vigo',
  'GIJ': 'Gijón', 'OVI': 'Oviedo', 'SAN': 'Santander', 'GRX': 'Granada',
  'MUR': 'Murcia', 'SCQ': 'Santiago', 'ELX': 'Elche', 'ALM': 'Almería',
  'HUE': 'Huelva', 'CAD': 'Cádiz', 'JAE': 'Jaén', 'CAS': 'Castellón',
  'LOG': 'Logroño', 'TOL': 'Toledo', 'LLE': 'Lleida', 'GER': 'Girona',
  'TAR': 'Tarragona', 'BUR': 'Burgos', 'LEO': 'León', 'SAL': 'Salamanca',
  'SEG': 'Segovia', 'AVI': 'Ávila', 'CAC': 'Cáceres', 'ALB': 'Albacete'
};
...
// ============================================================================
// VALIDACIONES FUNDAE ESPECÍFICAS
// ============================================================================

/**
 * Valida que un lugar de trabajo sea una provincia española o código IATA
 */
export function validateLugarTrabajo(value: string | null | undefined): ValidationResult {
  if (!value) {
    return {
      isValid: false,
      errorType: 'missing_value',
      errorMessage: 'Lugar de trabajo es obligatorio',
      severity: 'high'
    };
  }
  ...
  // 1. Verificar si es una provincia española
  if (PROVINCIAS_ESPANOLAS.includes(cleanValue)) {
    return { isValid: true };
  }

  // 2. Verificar si es un código IATA
  if (cleanValue in CODIGOS_IATA) {
    return { isValid: true };
  }
  ...
  return {
    isValid: false,
    errorType: 'invalid_lugar',
    errorMessage: `"${value}" no es una provincia española o código válido`,
    expectedFormat: 'Provincia española o código IATA (MAD, BCN, VLC, etc.)',
    severity: 'high'
  };
}

/**
 * Valida que una valoración sea Sí/No/NC (para preguntas 8.1, 8.2)
 * Formato formulario FUNDAE: 1=Sí, 2=No
 */
export function validateValoracionSiNo(value: any): ValidationResult {
  if (value === null || value === undefined || value === '') {
  ...
  // Valores válidos:
  // - 1 o "1" = Sí (formato formulario FUNDAE)
  // - 2 o "2" = No (formato formulario FUNDAE)
  // - Sí, No, S, N (texto directo)
  const valoresValidos = ['si', 'sí', 'no', 's', 'n', 'yes', '1', '2'];
  if (valoresValidos.includes(strValue)) {
    return { isValid: true };
  }

  // También aceptar números directos
  if (value === 1 || value === 2) {
    return { isValid: true };
  }
  ...
  return {
    ...
    errorMessage: `"${value}" debe ser 1 (Sí), 2 (No) o NC`,
    expectedFormat: '1=Sí, 2=No, o NC',
    severity: 'medium'
  };
}

/**
 * Valida que una valoración FUNDAE esté en rango 1-4 o sea NC
 */
export function validateValoracionFundae(value: any): ValidationResult {
  if (value === null || value === undefined || value === '') {
    return { isValid: true }; // Opcional
  }
  ...
  if (isNaN(numValue) || numValue < 1 || numValue > 4) {
    return {
      isValid: false,
      errorType: 'out_of_range',
      errorMessage: `"${value}" debe ser 1-4 o NC`,
      expectedFormat: '1-4 (1=Muy en desacuerdo, 4=Muy de acuerdo) o NC',
      severity: 'medium'
    };
  }
...
/**
 * Detecta si hay múltiples respuestas marcadas
 */
export function detectMultipleResponses(value: any): { hasMultiple: boolean; shouldMarkNC: boolean } {
  if (!value) {
    return { hasMultiple: false, shouldMarkNC: false };
  }

  // Si es array → múltiples respuestas
  if (Array.isArray(value) && value.length > 1) {
    return { hasMultiple: true, shouldMarkNC: true };
  }

  const strValue = String(value).trim();

  // Patrones de múltiples respuestas
  ...
  const hasMultiple = multiplePatterns.some(pattern => pattern.test(strValue));

  return { hasMultiple, shouldMarkNC: hasMultiple };
}

/**
 * Valida categoría profesional (debe ser 1-9)
 */
export function validateCategoriaProfesional(value: any): ValidationResult {
  if (!value || isNC(value)) {
    return { isValid: true };
  }
  ...
  if (isNaN(numValue) || numValue < 1 || numValue > 9) {
    return {
      ...
      errorMessage: `"${value}" no es una categoría válida (debe ser 1-9)`,
      expectedFormat: '1-9 (categoría profesional)',
      severity: 'high'
    };
  }
