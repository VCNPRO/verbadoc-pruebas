...
async function processSingleDocument(
  ...
  if (settings.autoClassify) {
    result.classification = await classifyDocument(file, 'gemini-2.5-flash');
  ...
  if (schema.length > 0) {
    ...
    result.cost! += settings.model === 'gemini-2.5-flash' ? 0.0005 : 0.0016;
  ...
  if (settings.autoValidate && result.extractedData && schema.length > 0) {
    result.validation = await validateExtractedData(result.extractedData, schema, file, 'gemini-2.5-flash');
...
async function processMultiDocumentPDF(
  ...
  for (const segment of result.segmentation.segments) {
    ...
    try {
      ...
      if (settings.autoClassify) {
        segmentResult.classification = await classifyDocument(segmentFile, 'gemini-2.5-flash');
        result.cost! += 0.0005;
      }
      ...
      if (settings.autoValidate && segmentResult.extractedData && schema.length > 0) {
        segmentResult.validation = await validateExtractedData(segmentResult.extractedData, schema, segmentFile, 'gemini-2.5-flash');
        result.cost! += 0.0005;
      }
